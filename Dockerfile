...
RUN curl -SL https://yum.oracle.com/RPM-GPG-KEY-oracle-ol7 > /etc/pki/rpm-gpg/RPM-GPG-KEY-oracle \
    ...
    && wget -O /tmp/cmake-3.22.3-Linux-x86_64.sh --no-check-certificate --quiet 'https://github.com/Kitware/CMake/releases/download/v3.22.3/cmake-3.22.3-linux-x86_64.sh' \
    && bash /tmp/cmake-3.22.3-Linux-x86_64.sh --prefix=/usr/local --exclude-subdir \
    && rm /tmp/cmake-3.22.3-Linux-x86_64.sh \
